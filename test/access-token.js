...
contract('AccessToken' , (accounts) => {
	...
	it('sale : should call eth to usd method and set new values' , async () => {
		await saleInstance.setEthToUsd(1000 , {from: owner});
		
		var ethToUsd = await saleInstance.ethToUsd.call();
		assert.equal(ethToUsd.toNumber() , 1000 , 'ethToUsd does not match');

		var usdToWei = await saleInstance.usdToWei.call();
		assert.equal(usdToWei.toNumber() , 1E18 / 1000 , 'usdToWei does not match');		

		var phase1Rate = await saleInstance.phase1Rate.call();
		assert.equal(phase1Rate.toNumber() , 0.020 * usdToWei.toNumber() , 'phase1Rate does not match');		

		var phase2Rate = await saleInstance.phase2Rate.call();
		assert.equal(phase2Rate.toNumber() , 0.025 * usdToWei.toNumber() , 'phase2Rate does not match');		

		var phase3Rate = await saleInstance.phase3Rate.call();
		assert.equal(phase3Rate.toNumber() , 0.025 * usdToWei.toNumber() , 'phase3Rate does not match');		
	});
	...
	it('sale : should allow user to buy tokena' , async () => {
	...
	it('sale : should allow owner to send remaining token to exchange if sale ended soft cap reached and hard cap not reached' , async () => {
		...
		assert.equal(tokenVestingDistributedBefore , false , 'tokenVestingDistributed reached should be false');

		await saleInstance.distributeTokens({from: owner});

		var tokenVestingDistributedBefore = await saleInstance.tokenVestingDistributed.call();
		assert.equal(tokenVestingDistributedBefore , true , 'tokenVestingDistributed reached should be false');
		...
		assert.equal(exchangeBalanceAfter.toNumber() , exchangeBalanceBefore.toNumber() + saleBalanceBefore.toNumber() , 'exchange contract balance should be increased');
	});

	// Send to exchange
	it('sale : should not allow owner to send remaining token to exchange if tokens not distributed' , async () => {
		var account = accounts[3];	

		var refundableBefore = await saleInstance.isRefundable.call();
		assert.equal(refundableBefore , false , 'refundable should be false');	

		var softcapReachedBefore = await saleInstance.tokenVestingDistributed.call();
		assert.equal(softcapReachedBefore , false , 'softcap reached should be false');
		...
		assert.equal(saleBalanceAfter.toNumber() , saleBalanceBefore.toNumber() , 'sale contract balance should be zero');
		assert.equal(exchangeBalanceAfter.toNumber() , exchangeBalanceBefore.toNumber() , 'exchange contract balance should be increased');
	});
