...
contract('IncentivePool', function (accounts) {
    const IncentivePool = artifacts.require('./../contracts/stub/IncentivePoolStub.sol');
    const Relay = artifacts.require('./../contracts/Relay.sol');
    const RelayStub = artifacts.require('./../contracts/stub/RelayStub.sol');
    ...
    const RelayConstructorParams = (_tokenAddr, _governanceAddr, _dmAddr) => {
        return Object.values({
            _tokenAddress: _tokenAddr,
            _governanceAddress: _governanceAddr,
            _decisionModuleAddress: _dmAddr
        });
    };
    ...
    async function deployContracts() {
        ...
        var fakeRelay = await RelayStub.new(...RelayConstructorParams(token.address, governance.address, decisionModule.address), {gas: 10000000});
        await fakeRelay.setGovernanceAddress(governanceAddr);
        await fakeRelay.setDecisionModuleAddress(decisionModuleAddr);
